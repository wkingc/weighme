...
Daily fluctuations are misleading (empirical evidence: a single urination event produced a -2 lb delta), so I employed statistical learning to reveal the true signal beneath the noise. Future weight loss now arrives with 95% confidence and 0% chill (@sec-appendix).
...
def weight_plot(data, pdata):
    ...
    plt.title('Line Plot with Groups')
    plt.xlabel('Day')
    plt.ylabel('Weight')
...
@eq-day-rate is can be used to determined the expected average weight loss per day.
